...
namespace Evade
{
    internal class Program
    {
        ...
        private static int _lastSentMovePacketT = 0;
        private static int _lastSentMovePacketT2 = 0;

        public static int LastSMovePacketT { get; } = 0;

        private static bool _forcePathFollowing = false;
            ...
            if (!alreadyAdded || skillshot.SpellData.DontCheckForDuplicates)
            {
                ...
                if (skillshot.DetectionType == DetectionType.ProcessSpell)
                {
                    ...
                    if (skillshot.SpellData.SpellName == "SyndraE" || skillshot.SpellData.SpellName == "syndrae5")
                    {
                        ...
                        positions.AddRange(from minion in ObjectManager.Get<Obj_AI_Minion>() where minion.Name == "Seed" && !minion.IsDead && (minion.Team != ObjectManager.Player.Team || Config.TestOnAllies) select Extensions.To2D((Vector3) minion.ServerPosition));
                ...
                if (ally.IsValidTarget(1000))
                {
                    ...
                    if (shieldAlly != null && shieldAlly.Cast<CheckBox>().CurrentValue)
                    {
                        ...
                        if (!allySafeResult.IsSafe)
                        {
                            ...
                            {
                                dangerLevel = Math.Max(dangerLevel, Config.SkillShotsMenu["DangerLevel" + skillshot.SpellData.MenuItemName].Cast<Slider>().CurrentValue);
                            }
